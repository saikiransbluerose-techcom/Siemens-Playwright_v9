...
// handle possible banner refresh
await maybeReload(page);
...
// 2) force it ON (use check(); fall back to click() if it's not a checkbox)
try {
...
// 3) the toggle often triggers a partial refresh; handle it
await maybeReload(page);

// 4) wait for the email input to attach; retry once if needed
let emailInput = u0.gadget1.getByTestId('20160908075844034240762');
await emailInput.waitFor({ state: 'attached', timeout: 30000 }).catch(async () => {
