// SiemensApplication/tests/flowWithTestData.spec.js  (ESM)
import fs from 'fs';
...
test('test', async ({ page }) => {
  ...
  if (!fs.existsSync(dataPath)) throw new Error(`Excel not found at: ${dataPath}`);

  // --- Read sheets ---
  const SW          = getSheetMap(dataPath, 'StartWorkflow');  // { field: value }
  const ASSIGNEES   = getSheetRows(dataPath, 'Assignees');     // rows: [{searchText, displayText, emailIfAny}, ...]
  const APPROVALS   = getSheetRows(dataPath, 'Approvals');     // rows per approver; headers must match your Excel
...
//   // Any additional assignees -> Add user (your pattern)
//   for (let i = 3; i < ASSIGNEES.length; i++) {
//     await u0.gadget1.getByTestId('20150908171228012736690').click();
//     await u0.gadget1.getByRole('link', { name: 'Add user', exact: true }).click();
//     await u0.gadget1.getByRole('textbox', { name: 'Lastname, Firstname,' }).fill(getAssigneeId(i));
//     await u0.gadget1.getByText(getAssigneeName(i)).click();
//     const email = getAssigneeEmail(i);
//     if (email) {
//       await u0.gadget1.getByTestId('201609080744500837333').click();
//       await u0.gadget1.getByTestId('20160908075844034240762').click();
//       await u0.gadget1.getByTestId('20160908075844034240762').fill(email);
//     }
//   }
